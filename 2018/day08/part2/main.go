...
--- Part Two ---
The second check is slightly more complicated: you need to find
the value of the root node (A in the example above).
...
func main() {
	...
	err := t.Setup(f)
	if err != nil {
		fmt.Printf("got error setting up graph: %v", err)
		os.Exit(1)
	}

	got := t.SumRoot()
	fmt.Printf("sum root: %v\n", got)
}
