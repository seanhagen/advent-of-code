...
import (
	...
	day4 "github.com/seanhagen/advent-of-code/2020/day04"
...
func main() {
	...
	passLines := day4.ParseFile(in)

	fmt.Printf("Have %v passports to process.\n", len(passLines))

	for _, v := range passLines {
		pp, err := day4.NewPassport(v, true)
		pp.SetStrict(true)
		if err != nil {
			fmt.Printf("Unable to create passport: %v\n\n", err)
			os.Exit(1)
		}
		if pp.IsValid() {
			nv++
			...
			// } else {
			// 	// fmt.Printf("Error parsing passport '%v'\n", v)
			// 	fmt.Printf("Passport: %v\n", v)
			// 	pp.PrintErrors()
		}
		// fmt.Printf("----------------\n")
