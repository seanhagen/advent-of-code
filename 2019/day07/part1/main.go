...
that should run on your existing Intcode computer. Each amplifier will need to run a
copy of the program.
...
instruction. (If the amplifier has not yet received an input signal, it waits until
one arrives.)
...
mean setting amplifier A to phase setting 3, amplifier B to setting 1, C to 2, D to 4, and
E to 0. Then, you could determine the output signal that gets sent from amplifier E to the
...
func main() {
	...
	day07.Permutations([]int{0, 1, 2, 3, 4}, func(p []int) {
