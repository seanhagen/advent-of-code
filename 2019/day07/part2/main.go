...
import (
	...
	"github.com/seanhagen/advent-of-code/2019/day07"
)
...
to amplifier B's input, and so on. However, the output from amplifier E is now connected into
amplifier A's input. This creates the feedback loop: the signal will be sent through the
amplifiers many times.

In feedback loop mode, the amplifiers need totally different phase settings: integers from 5
to 9, again each used exactly once. These settings will cause the Amplifier Controller Software
to repeatedly take input and produce output many times before halting. Provide each amplifier
its phase setting at its first input instruction; all further input/output instructions are
for signals.
...
func main() {
	...
	day07.Permutations([]int{5, 6, 7, 8, 9}, func(p []int) {
		ac, err := day07.NewAmpCircuit(code, p)
		if err != nil {
			fmt.Printf("unable to create amp circuit: %v\n", err)
			return
		}
		ac.SetMode(day07.ModeFeedback)

		out, err := ac.Run()
		if err != nil {
			fmt.Printf("unable to run amp circuit: %v\n", err)
			return
		}
	...
	fmt.Printf("max thruster signal: %v\n", mts)
