...
import (
	...
	"github.com/seanhagen/advent-of-code/2019/day08"
)
...
fill each row of the image left-to-right, then move downward to the next row, filling rows top-to-bottom
...
func main() {
	bits, err := ioutil.ReadFile("../input.txt")
	if err != nil {
		fmt.Printf("unable to read input: %v\n", err)
		os.Exit(1)
	}

	img, err := day08.NewImage(string(bits), width, height)
	if err != nil {
		fmt.Printf("unable to create image: %v\n", err)
		os.Exit(1)
	}
