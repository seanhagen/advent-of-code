...
can hold any integer. However, it seems to make use of only a few instructions:
	 ...
	 -	cpy x y copies x (either an integer or the value
      of a register) into register y.
	 ...
	 - jnz x y jumps to an instruction y away (positive
	   means forward; negative means backward), but only
     if x is not zero.
...
then skip the last dec a (because a is not zero, so the jnz a 2 skips it), leaving register a at 42.
